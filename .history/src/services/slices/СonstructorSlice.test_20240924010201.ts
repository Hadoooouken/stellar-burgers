...
describe('constructorSlice reducer', () => {
  it('add ingredient', () => {
    const initialState: { bun: null; ingredients: TConstructorIngredient[] } = { bun: null, ingredients: [] };
    
    const action: { type: string; payload: TConstructorIngredient } = addIngredient({
      _id: 'test-id',
  ...
  it('remove ingredient by id', () => {
    ...
    const action: { type: string; payload: { id: string } } = removeIngredient({ id: 'test-id' });
    const newState = constructorReducer(initialState, action);
