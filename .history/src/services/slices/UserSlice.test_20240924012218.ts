...
describe('User Slice - Async Actions', () => {
  ...
  it('should handle loginUser thunk', async () => {
    ...
    // Мокаем API вызов
    jest.spyOn(global, 'loginUserApi').mockResolvedValueOnce({ user, accessToken: 'token', refreshToken: 'refresh' });
  ...
  it('should handle loginUser rejected', async () => {
    ...
    jest.spyOn(global, 'loginUserApi').mockRejectedValueOnce(new Error('Login failed'));
