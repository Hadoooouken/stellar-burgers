...
export const fetchOrderByNumber = createAsyncThunk<TOrder, number>(
  ...
  async (number, { rejectWithValue }) => {
    try {
      const response = await getOrdersApi(); // Используем запрос для получения всех заказов или одного по номеру
      const order = response.orders.find((order) => order.number === number);
