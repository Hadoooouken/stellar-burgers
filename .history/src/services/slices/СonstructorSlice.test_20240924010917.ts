...
describe('constructorSlice reducer', () => {
  ...
  it('should return the initial state', () => {
    ...
    expect(newState).toEqual(createInitialState()); // Проверка на соответствие начального состояния
  });

  it('add ingredient', () => {
    const initialState = createInitialState();
    const action = addIngredient(createIngredient('test-id')); // Здесь используется test-id для _id
  ...
  it('remove ingredient by id', () => {
    ...
    expect(newState.ingredients.length).toBe(0); // Проверка, что длина массива ингредиентов 0
  });
  ...
  it('move ingredient up', () => {
    ...
    expect(newState.ingredients[0].id).toBe('2'); // Проверка, что первый ингредиент теперь '2'
  });
  ...
  it('move ingredient down', () => {
    ...
    expect(newState.ingredients[0].id).toBe('2'); // Проверка, что первый ингредиент теперь '2'
    expect(newState.ingredients[1].id).toBe('1'); // Проверка, что второй ингредиент теперь '1'
  });
