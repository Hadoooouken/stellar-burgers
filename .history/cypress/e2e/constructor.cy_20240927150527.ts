...
describe('Burger Constructor Page', () => {
  ...
  it('should open and close ingredient modal', () => {
    ...
    cy.get('[data-testid="modal"]').should('be.visible').and('contain', 'Биокотлета из марсианской Магнолии');
  ...
  it('should create an order and clear constructor', () => {
    ...
    cy.wait('@createOrder');
    cy.get('[data-testid="modal"]').should('be.visible').and('contain', 'Ваш заказ успешно принят!');
    cy.get('[data-testid="order-number"]').should('contain', '12345');
