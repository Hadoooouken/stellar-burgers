...
describe('add ingredients to constructor', function () {
  beforeEach(() => {
    cy.intercept('GET', 'https://norma.nomoreparties.space/api/ingredients').as(
      'getIngredients'
    );
    cy.visit('/');
    cy.wait('@getIngredients'); // Ждем, пока данные загрузятся
    it('add buns to constructor', function () {
      cy.contains('Выберите булки').should('exist');
      cy.get('body').then(($body) => {
        console.log($body.html()); // Выводим HTML-код страницы в консоль
      });
      cy.get('[data-cy=bun-ingredients]').contains('Добавить').click();
      // Дальше ваш код
    });
