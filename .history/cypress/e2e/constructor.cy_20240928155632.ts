...
//       cy.get('.constructor-element').contains('Соус с шипами Антарианского плоскоходца Spicy-X').should('exist');
//     });
//   });
...
describe('Constructor', () => {
  beforeEach(() => {
    ...
    // Логирование данных, получаемых с API
    cy.wait('@getIngredients').then((interception) => {
      console.log('Ingredients Data:', interception.response.body);
      cy.log('Ingredients Data:', JSON.stringify(interception.response.body));
    });
  });
  ...
  describe('Adding Ingredients to the burger constructor', () => {
    ...
    it('should add sauces to the burger constructor', () => {
      ...
      cy.get('[data-testid=category-sauce]').contains('Добавить').click();
      cy.get('.constructor-element')
        .contains('ССоус фирменный Space Sauceоус с шипами Антарианского плоскоходца Spicy-X')
        .should('exist');
    });
  });
  ...
  describe('Order Processing', () => {
    ...
    it('should successfully create an order', () => {
      ...
      cy.wait('@createOrder', { timeout: 50000 })
        .its('response.statusCode')
        .should('eq', 200);
