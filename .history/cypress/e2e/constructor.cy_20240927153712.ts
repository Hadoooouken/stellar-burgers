describe('BurgerConstructor', () => {
  beforeEach(() => {
    // Перехватываем запросы на получение ингредиентов и заказов, а также данные пользователя
    cy.intercept('GET', 'api/ingredients', { fixture: 'ingredients.json' });
    cy.intercept('POST', 'api/orders', { fixture: 'order.json' }).as(
      'makeOrder'
    );
    cy.intercept('GET', 'api/auth/user', { fixture: 'logub.json' }).as(
      'getUser'
    );

    // Устанавливаем токены и куки
    cy.setCookie('accessToken', 'testToken');
    localStorage.setItem('refreshToken', 'testRefreshToken');

    // Переходим на главную страницу и устанавливаем размеры экрана
    cy.visit('/');
