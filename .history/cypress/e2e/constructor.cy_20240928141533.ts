describe('Constructor', () => {
  ...
  describe('Order Processing', () => {
    ...
    it('should show alert when trying to create order without ingredients', () => {
      ...
      // Нажимаем на кнопку оформления заказа без добавления ингредиентов
      cy.get('[type=button]').contains('Оформить заказ').click();

      // Проверяем, что alert был вызван
      cy.get('@alertStub').should('be.called');

      // Проверяем, что alert был вызван с нужным текстом
      cy.get('@alertStub').should('be.calledWith', 'Выберите булки и начинки для оформления заказа');
    });
  });
});
