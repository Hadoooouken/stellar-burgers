describe('Burger Constructor Page', () => {
  ...
  it('should display ingredients and add them to constructor', () => {
    ...
    cy.get('[data-testid="category-bun"] [data-testid="ingredient"]').should('have.length.greaterThan', 0);
    
    // Добавляем булку в конструктор
    cy.get('[data-testid="category-bun"] [data-testid="ingredient"]').first().drag('[data-testid="constructor-drop-area"]');
    cy.get('[data-testid="constructor-bun"]').should('contain', 'верх');
    
    // Добавляем начинку
    cy.get('[data-testid="category-main"] [data-testid="ingredient"]').first().drag('[data-testid="constructor-drop-area"]');
    cy.get('[data-testid="constructor-ingredient"]').should('have.length', 1);
  });
  
  it('should open and close ingredient modal', () => {
    // Открываем модалку ингредиента
    cy.get('[data-testid="category-bun"] [data-testid="ingredient"]').first().click();
