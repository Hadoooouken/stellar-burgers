...
describe('Constructor', () => {
  ...
  describe('Order Processing', () => {
    ...
    it('should show alert when trying to create order without ingredients', () => {
      ...
      cy.get('@alertStub').should('be.called'); // Проверка, что алерт был вызван
      cy.get('@alertStub').should('be.calledWith', 'Нельзя создать заказ без ингредиентов'); // Проверка текста алерта
    });
  });
});
