describe('Constructor', () => {
  ...
  const getRandomIngredient = (category: string): Cypress.Chainable<TIngredient> => {
    return cy.request('/api/ingredients').then((response) => {
      const ingredients: TIngredient[] = response.body.data.filter((ingredient: TIngredient) => ingredient.type === category);
  ...
  describe('Adding Ingredients to the burger constructor', () => {
    it('should add buns to the burger constructor', () => {
      ...
      getRandomIngredient('bun').then((randomBun) => {
        cy.get('[data-testid=category-bun]').contains('Добавить').click();
        cy.get('.constructor-element_pos_top').contains(randomBun.name).should('exist');
      });
    });

    it('should add ingredients to the burger constructor', () => {
      cy.contains('Выберите начинку').should('exist');
      getRandomIngredient('main').then((randomIngredient) => {
        cy.get('[data-testid=category-main]').contains('Добавить').click();
        cy.get('.constructor-element').contains(randomIngredient.name).should('exist');
      });
    });

    it('should add sauces to the burger constructor', () => {
      cy.contains('Выберите начинку').should('exist');
      getRandomIngredient('sauce').then((randomSauce) => {
        cy.get('[data-testid=category-sauce]').contains('Добавить').click();
        cy.get('.constructor-element').contains(randomSauce.name).should('exist');
      });
    });
  });
